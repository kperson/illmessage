...
object  Main {
  ...
  def run(runtimeApiEndpoint: String, handler: RequestStreamHandler) {
    runRequest("GET", s"http://abc/2018-06-01/runtime/invocation/next")
      .foreach { rs =>
      val requestId = rs.lowerHeaders("Lambda-Runtime-Aws-Request-Id".toLowerCase)
      try {
        val out = new ByteArrayOutputStream()
        handler.handleRequest(new ByteArrayInputStream(rs.body), out, null)
        runRequest(
          "POST",
