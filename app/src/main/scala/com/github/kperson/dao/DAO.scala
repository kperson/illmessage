...
trait DAO {
  ...
  def deliverMessage(message: Message, subscription: Subscription): Future[Either[Boolean, DeadLetterMessage]]

  def saveDeadLetterMessage(message: DeadLetterMessage): Future[Any]
